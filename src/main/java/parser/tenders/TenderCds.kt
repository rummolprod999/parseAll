...
class TenderCds(val tn: Cds) : TenderAbstract(), ITender {
    init {
        etpName = "ООО «ЦДС»"
        etpUrl = "https://tender.cds.spb.ru/"
    }

    val typeFz by lazy { 355 }
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(
                fun(con: Connection) {
                    ...
                    insertTender.setString(14, tn.contact)
                    ...
                    if (tn.delivTerm != "") {
                        con.prepareStatement(
                            ...
                            .apply {
                                setInt(1, idLot)
                                setInt(2, idCustomer)
                                setString(3, "Объект: ${tn.delivPlace}")
                                setString(4, "Сроки проведения тендера: ${tn.delivTerm}")
                                executeUpdate()
                                close()
                            }
                    }
