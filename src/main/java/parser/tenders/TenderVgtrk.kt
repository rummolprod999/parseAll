...
class TenderVgtrk(val tn: Vgtrk) : TenderAbstract(), ITender {
    init {
        etpName = "Государственный интернет-канал \"Россия\""
        etpUrl = "https://tendering.vgtrk.com/"
    }

    companion object TypeFz {
        const val typeFz = 258
    }
    ...
    override fun parsing() {
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(fun(con: Connection) {
                ...
                var idPlacingWay = 0
                var idTender = 0
                val pwName = htmlTen.selectFirst("dt:contains(Способ закупки) + dd")?.ownText()?.trim { it <= ' ' }
                    ?: ""
                if (pwName != "") {
                ...
                insertTender.setString(14, tn.state)
                ...
                val documents: Elements = htmlTen.select("a[href^='/docs/']")
                documents.forEach { doc ->
                    val hrefT = doc?.attr("href")?.trim { it <= ' ' } ?: ""
                    val href = "https://tendering.vgtrk.com$hrefT"
