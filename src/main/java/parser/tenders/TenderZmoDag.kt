...
class TenderZmoDag(val tn: ZmoKursk) : TenderAbstract(), ITender {
    init {
        etpName = "Электронный маркет Республики Дагестан"
        etpUrl = "https://marketrd.rts-tender.ru/"
    }
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(
                fun(con: Connection) {
                    ...
                    val idPlacingWay = 0
                    var idTender = 0
                    val idRegion = getIdRegion(con, "дагест")
                    val insertTender =
                        con.prepareStatement(
