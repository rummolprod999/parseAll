...
class TenderAchi(
    ...
    override fun parsing() {
        ...
        val status =
            htmlTen.selectFirst("div.tender__item__status > span")?.text()?.trim { it <= ' ' } ?: ""
        val dates =
            htmlTen
                .selectFirst("div.tender__item__client__info:contains(Подача предложений:) span")
                ?.text()
                ?.trim { it <= ' ' }
                ?: run {
                    logger("dates was not found", tn.href)
                    throw Exception("dates was not found")
                }
        val pubDateT = dates.getDataFromRegexp("(.+)-").replaceDateAchi()
        val datePub = pubDateT.getDateFromString(formatterAchi)
        if (datePub == Date(0L)) {
            run {
                logger("datePub was not found", tn.href)
        ...
        if (dateEnd == Date(0L)) {
            run {
                logger("dateEnd was not found", tn.href)
                throw Exception("dateEnd was not found")
            }
        }
        DriverManager
            ...
            .use(
                fun(con: Connection) {
                    ...
                    val lots =
                        htmlTen.select(
                            "div.tender__item__list div.tender__item__info__title.lot-list ~ div.tender__item__list__item",
                        )
    ...
    private fun insertDocs(
        ...
        val documents =
            htmlTen.select("div.tender__item__list.docs-list div.tender__item__list__item a")
