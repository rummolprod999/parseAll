...
class TenderAchi(val tn: Achi) : TenderAbstract(), ITender {
    ...
    override fun parsing() {
        ...
        val status = htmlTen.selectFirst("div.tender__item__status > span")?.text()?.trim { it <= ' ' }
            ?: ""
        val dates = htmlTen.selectFirst("div.tender__item__client__info:contains(Подача предложений:) span")?.text()
            ?.trim { it <= ' ' }
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(fun(con: Connection) {
                val stmt0 =
                    con.prepareStatement("SELECT id_tender FROM ${BuilderApp.Prefix}tender WHERE purchase_number = ? AND type_fz = ? AND end_date = ? AND doc_publish_date = ? AND notice_version = ?")
                        .apply {
                ...
                if (fullnameOrg != "") {
                    ...
                    if (rso.next()) {
                        ...
                        val postalAdr = htmlTen.selectFirst("span:contains(Адрес) + div")?.text()?.trim { it <= ' ' }
                            ?: ""
                        inn =
                            htmlTen.selectFirst("span:contains(Фискальный код/IDNO) + div")?.text()?.trim { it <= ' ' }
                                ?: ""
                        val kpp = ""
                        val email = htmlTen.selectFirst("span:contains(Эл. адрес) + div")?.text()?.trim { it <= ' ' }
                            ?: ""
                        val phone =
                            htmlTen.selectFirst("span:contains(Контактный номер) + div")?.text()?.trim { it <= ' ' }
                                ?: ""
                ...
                val lots =
                    htmlTen.select("div.tender__item__list div.tender__item__info__title.lot-list ~ div.tender__item__list__item")
                for ((ind, l) in lots.iterator().withIndex()) {
                    val nmckT =
                        l.selectFirst("a + div:contains(Бюджет:)")?.text()?.replace("Бюджет:", "")?.trim { it <= ' ' }
                            ?: ""
