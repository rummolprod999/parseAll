...
class TenderSalavat(val tn: Salavat) : TenderAbstract(), ITender {
    companion object TypeFz {
        val typeFz = 75
    }

    init {
        etpName = "ООО «Газпром нефтехим Салават»"
        etpUrl = "http://salavat-neftekhim.gazprom.ru"
    }
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb)
            .use(
                fun(con: Connection) {
                    ...
                    val documents: Elements = htmlTen.select("div.tender_description ~ p span a")
