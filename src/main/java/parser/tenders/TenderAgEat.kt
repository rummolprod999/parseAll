...
class TenderAgEat(val tn: AgEat, val driver: ChromeDriver) : TenderAbstract(), ITender {
    ...
    override fun parsing() {
        ...
        val datePubTmp =
            driver
                .findElementWithoutException(By.xpath("//div[@id = 'publishDate']"))
                ?.text
                ?.trim()
                ?.trim { it <= ' ' }
                ?: ""
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(
                fun(con: Connection) {
                    ...
                    val inn =
                        driver
                            .findElementWithoutException(By.xpath("//div[@id = 'customerInn']"))
                            ?.text
                            ?.trim()
                            ?.trim { it <= ' ' }
                            ?: ""
                    ...
                    val nmck =
                        driver
                            .findElementWithoutException(By.xpath("//div[@id = 'lotPrice']"))
                            ?.text
                            ?.extractPrice()
                            ?.deleteAllWhiteSpace()
                            ?.replace(",", ".")
                    ...
                    var delivTerm4 =
                        driver
                            .findElementWithoutException(By.xpath("//div[@id = 'lotDeliveryDate']"))
                            ?.text
                            ?.trim()
                            ?.trim { it <= ' ' }
                            ?: ""
                    ...
                    driver.switchTo().defaultContent()
                    val purObjts =
                        driver.findElements(By.xpath("//tr[contains(@id, 'lotItemRow')]"))
