    ...
    override fun parsing() {
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(fun(con: Connection) {
                val stmt0 =
                    con.prepareStatement("SELECT id_tender FROM  ${BuilderApp.Prefix}tender WHERE purchase_number = ? AND date_version = ? AND type_fz = ?")
                ...
                var dateEndT =
                    html.selectFirst("p:has(b:containsOwn(Запланированная дата окончания сбора предложений:))")
                        ?.ownText()?.trim() ?: ""
                if (dateEndT != "") {
                    val dte =
                        dateEndT.replace("Запланированная дата окончания сбора предложений:", "").trim { it <= ' ' }
                ...
                if (fullnameOrg != "") {
                    ...
                    if (rso.next()) {
                        ...
                        val stmtins = con.prepareStatement(
                            "INSERT INTO ${BuilderApp.Prefix}organizer SET full_name = ?",
                ...
                val insertTender = con.prepareStatement(
                    "INSERT INTO ${BuilderApp.Prefix}tender SET id_region = 0, id_xml = ?, purchase_number = ?, doc_publish_date = ?, href = ?, purchase_object_info = ?, type_fz = ?, id_organizer = ?, id_placing_way = ?, id_etp = ?, end_date = ?, cancel = ?, date_version = ?, num_version = ?, notice_version = ?, xml = ?, print_form = ?",
                    Statement.RETURN_GENERATED_KEYS
                )
                ...
                if (fullnameOrg != "") {
                    ...
                    if (rso.next()) {
                        ...
                        stmto.close()
                        val stmtins = con.prepareStatement(
                            "INSERT INTO ${BuilderApp.Prefix}customer SET full_name = ?, is223=1, reg_num = ?",
                            Statement.RETURN_GENERATED_KEYS
                        )
                        stmtins.setString(1, fullnameOrg)
                ...
                val purObj: Elements =
                    html.getElementsByAttributeValueEnding("itemtype", "https://schema.org/OfferCatalog")
                purObj.forEach { po ->
                    val p = po.select("div")
                    p.forEach { ppp ->
                ...
                val delivPay = html.selectFirst("p:has(b:containsOwn(Условия оплаты:))")?.ownText()?.trim { it <= ' ' }
                    ?: ""
                val delivPlace =
                    html.selectFirst("p:has(b:containsOwn(Условия поставки:))")?.ownText()?.trim { it <= ' ' }
                        ?: ""
                var dTerm = ""
                if (delivDate != "") {
                ...
                if (dTerm != "") {
                    val insertCusRec =
                        con.prepareStatement("INSERT INTO ${BuilderApp.Prefix}customer_requirement SET id_lot = ?, id_customer = ?, delivery_term = ?, delivery_place = ?")
