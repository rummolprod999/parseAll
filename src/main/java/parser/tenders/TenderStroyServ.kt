...
class TenderStroyServ(val tn: StroyServ) : TenderAbstract(), ITender {
    init {
        etpName = "АО «Стройсервис»"
        etpUrl = "https://stroyservis.com/"
    }

    val typeFz by lazy { 363 }
    ...
    private fun insertDocs(htmlTen: Document, con: Connection, idTender: Int) {
        val documents = htmlTen.select("div.standards-for-activities-inner")
        for (doc in documents) {
            val urlT = doc?.selectFirst("a")?.attr("href")?.trim { it <= ' ' } ?: ""
            val url = "https://stroyservis.com$urlT"
            var docName = doc?.selectFirst("p")?.text()?.trim { it <= ' ' } ?: ""
            if (docName == "") {
                docName = url
            }
            val insertDoc =
