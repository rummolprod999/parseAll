            ...
            .use(fun(con: Connection) {
                ...
                if (tn.orgName != "") {
                    ...
                    if (rso.next()) {
                        ...
                        stmto.close()
                        val postalAdr =
                            htmlTen.selectFirst("td:contains(Почтовый адрес) + td")?.ownText()?.trim { it <= ' ' }
                                ?: ""
                        val factAdr = ""
                        val inn = ""
                        val kpp = ""
                        val email =
                            htmlTen.selectFirst("td:contains(Адрес электронной почты) + td a")?.ownText()
                                ?.trim { it <= ' ' }
                                ?: ""
                        val phone =
                            htmlTen.selectFirst("td:contains(Номер контактного телефона) + td")?.ownText()
                                ?.trim { it <= ' ' }
                                ?: ""
                        val contactPerson = ""
                ...
                val documents: Elements = htmlTen.select("a[href^='http://files.bp.zakazrf.ru/File']")
                documents.forEach { doc ->
                    val href = doc?.attr("href")?.trim { it <= ' ' } ?: ""
                ...
                var delivTerm = htmlTen.selectFirst("td:contains(Сроки поставки) + td")?.ownText()
                    ?.trim { it <= ' ' }
                    ?: ""
