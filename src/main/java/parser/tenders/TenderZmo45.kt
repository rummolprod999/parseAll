...
class TenderZmo45(val tn: ZmoKursk) : TenderAbstract(), ITender {

    init {
        etpName = "ЭЛЕКТРОННЫЙ МАГАЗИН ЗАКУПОК МАЛОГО ОБЪЁМА Г. МИЧУРИНСК"
        etpUrl = "https://zmo-g45.rts-tender.ru/"
    }

    override fun parsing() {
        val (status, purNum, purObj, nmck, pubDate, endDate, url) = tn
        val dateVer = Date()
        DriverManager.getConnection(UrlConnect, UserDb, PassDb)
            .use(
                fun(con: Connection) {
                    val stmt0 =
                    ...
                    if (fullnameOrg != "") {
                        val stmto =
                            con.prepareStatement(
                                "SELECT id_organizer FROM ${Prefix}organizer WHERE full_name = ?"
                            )
                    ...
                    val idPlacingWay = 0
                    var idTender = 0
                    val idRegion = getIdRegion(con, "тамбов")
                    val insertTender =
                        con.prepareStatement(
                    ...
                    // val documents: Elements = htmlTen.select("h1:containsOwn(Документы закупки) +
                    // div ")
                    try {
                    ...
                    if (fullnameOrg != "") {
                        val stmtoc =
                            con.prepareStatement(
                                "SELECT id_customer FROM ${Prefix}customer WHERE full_name = ? LIMIT 1"
                            )
                    ...
                    val purobj1 =
                        htmlTen.select("table:contains(Код классификатора) + div table tbody tr")
                    purobj1.forEach { element ->
                        ...
                        val okei =
                            element.selectFirst("td:eq(3)")?.ownText()?.trim { it <= ' ' } ?: ""
                        val quantity =
