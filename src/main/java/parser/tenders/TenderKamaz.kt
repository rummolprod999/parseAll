...
class TenderKamaz(val tn: Kamaz) : TenderAbstract(), ITender {
    ...
    init {
        etpName = "ПАО “КАМАЗ”"
        etpUrl = "https://kamaz.ru/"
    }

    override fun parsing() {
        val dateVer = Date()
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb).use(fun(con: Connection) {
            val stmt0 = con.prepareStatement("SELECT id_tender FROM ${BuilderApp.Prefix}tender WHERE purchase_number = ? AND type_fz = ? AND doc_publish_date = ?").apply {
