        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(fun(con: Connection) {
                ...
                if (orgName != "") {
                    ...
                    var delivTerm1 = htmlTen.selectFirst("div:containsOwn(Регион доставки:)")?.ownText()
                        ?.trim { it <= ' ' }
                        ?: ""
                    var delivTerm2 = htmlTen.selectFirst("div:containsOwn(Адрес доставки:)")?.ownText()
                        ?.trim { it <= ' ' }
                        ?: ""
                    var delivTerm3 = htmlTen.selectFirst("div:containsOwn(Условия и сроки доставки:)")?.ownText()
                        ?.trim { it <= ' ' }
                        ?: ""
                    var delivTerm4 = htmlTen.selectFirst("div:containsOwn(Налог:)")?.ownText()
                        ?.trim { it <= ' ' }
                        ?: ""
                    var delivTerm5 = htmlTen.selectFirst("div:containsOwn(Способ оплаты:)")?.ownText()
                        ?.trim { it <= ' ' }
                        ?: ""
                    var delivTerm6 = htmlTen.selectFirst("div:containsOwn(Условия и сроки оплаты:)")?.ownText()
                        ?.trim { it <= ' ' }
                        ?: ""
                    val delivTerm =
                        "${delivTerm1}\n${delivTerm2}\n${delivTerm3}\n${delivTerm4}\n${delivTerm5}\n${delivTerm6}".trim { it <= ' ' }
