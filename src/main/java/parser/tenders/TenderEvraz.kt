...
class TenderEvraz(val tn: Evraz, val driver: ChromeDriver) : TenderAbstract(), ITender {
    ...
    override fun parsing() {
        ...
        driver.switchTo().defaultContent()
        val datePubTmp =
            driver.findElementWithoutException(By.xpath("//p[contains(., 'Дата публикации:')]/strong"))?.text?.trim()
                ?.trim { it <= ' ' }
                ?: ""
        ...
        var dateEndTmp =
            driver.findElementWithoutException(By.xpath("//p[contains(., 'Дата продления Технической части:')]/strong"))?.text?.trim()
                ?.trim { it <= ' ' }
                ?: ""
        if (dateEndTmp == "") {
            dateEndTmp =
                driver.findElementWithoutException(By.xpath("//p[contains(., 'Дата принятия Технической части:')]/strong"))?.text?.trim()
                    ?.trim { it <= ' ' }
                    ?: ""
        }
        if (dateEndTmp == "") {
            dateEndTmp =
                driver.findElementWithoutException(By.xpath("//p[contains(., 'Дата принятия Коммерческой части:')]/strong"))?.text?.trim()
                    ?.trim { it <= ' ' }
                    ?: ""
        }
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(fun(con: Connection) {
                ...
                var idPlacingWay = 0
                val noticeVer =
                    driver.findElementWithoutException(By.xpath("//p[contains(., 'Дополнительная информация:')]/strong"))?.text?.trim()
                        ?.trim { it <= ' ' }
                        ?: ""
                val idRegion = 0
                var idTender = 0
                ...
                var idCustomer = 0
                val delivTerm1 =
                    driver.findElementWithoutException(By.xpath("//p[contains(., 'Дата начала работ:')]/strong"))?.text?.trim()
                        ?.trim { it <= ' ' }
                val delivTerm2 =
                    driver.findElementWithoutException(By.xpath("//p[contains(., 'Дата окончания работ:')]/strong"))?.text?.trim()
                        ?.trim { it <= ' ' }
                val delivTerm = "Дата начала работ: $delivTerm1 Дата окончания работ: $delivTerm2".trim()
                if (delivTerm != "") {
                    con.prepareStatement("INSERT INTO ${BuilderApp.Prefix}customer_requirement SET id_lot = ?, id_customer = ?, delivery_term = ?")
                        .apply {
                            setInt(1, idLot)
                            setInt(2, idCustomer)
                ...
                con.prepareStatement("INSERT INTO ${BuilderApp.Prefix}purchase_object SET id_lot = ?, id_customer = ?, name = ?")
                    .apply {
                        setInt(1, idLot)
                        setInt(2, idCustomer)
                        setString(3, purObj)
