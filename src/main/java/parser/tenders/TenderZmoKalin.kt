...
class TenderZmoKalin(val tn: ZmoKursk) : TenderAbstract(), ITender {
    init {
        etpName = "Электронный магазин Калининградской области для закупок малого объёма"
        etpUrl = "https://market.gov39.ru/"
    }
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(
                fun(con: Connection) {
                    ...
                    val idPlacingWay = 0
                    var idTender = 0
                    val idRegion = getIdRegion(con, "калинин")
                    val insertTender =
                        con.prepareStatement(
