...
class TenderKurganKhim(val tn: KurganKhim) : TenderAbstract(), ITender {
    ...
    override fun parsing() {
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb).use(fun(con: Connection) {
            ...
            var idTender = 0
            val idRegion = 0
            val lotName = htmlTen.selectFirst("td:contains(Описание) + td span")?.text()?.trim { it <= ' ' }
                    ?: ""
    ...
    private fun getPurObjs(it: Element, con: Connection, idLot: Int, idCustomer: Int) {
        ...
        val urlLot = it.selectFirst("td:eq(0) span a")?.attr("href")?.trim { it <= ' ' }
                ?: run { logger("urlLot not found on"); return }
        ...
        val htmlTen = Jsoup.parse(pageTen)
        val delivTerm0 = htmlTen.selectFirst("td:contains(Требуемые сроки поставки) + td span")?.text()?.trim { it <= ' ' }
                ?: ""
        val delivTerm1 = htmlTen.selectFirst("td:contains(Общие условия оплаты) + td span")?.text()?.trim { it <= ' ' }
                ?: ""
        val delivTerm2 = htmlTen.selectFirst("td:contains(Особые требования) + td span")?.text()?.trim { it <= ' ' }
                ?: ""
        val delivTerm = "Требуемые сроки поставки: $delivTerm0\n Общие условия оплаты: $delivTerm1\n Особые требования: $delivTerm2".trim { it <= ' ' }
