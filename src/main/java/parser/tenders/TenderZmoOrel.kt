...
class TenderZmoOrel(val tn: ZmoKursk) : TenderAbstract(), ITender {
    init {
        etpName = "ЗАКУПКИ МАЛОГО ОБЪЕМА ОРЛОВСКОЙ ОБЛАСТИ"
        etpUrl = "https://zmo-orel.rts-tender.ru/"
    }
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(
                fun(con: Connection) {
                    ...
                    val idPlacingWay = 0
                    var idTender = 0
                    val idRegion = getIdRegion(con, "орлов")
                    val insertTender =
                        con.prepareStatement(
