...
class TenderBrn(val tn: ZmoKursk) : TenderAbstract(), ITender {
    init {
        etpName = "ЭЛЕКТРОННЫЙ МАГАЗИН БРЯНСКОЙ ОБЛАСТИ"
        etpUrl = "https://zm-tender32.rts-tender.ru/"
    }
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(
                fun(con: Connection) {
                    ...
                    val idPlacingWay = 0
                    var idTender = 0
                    val idRegion = getIdRegion(con, "брян")
                    val insertTender =
                        con.prepareStatement(
