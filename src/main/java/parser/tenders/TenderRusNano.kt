        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb).use(fun(con: Connection) {
            ...
            if (tn.orgName != "") {
                ...
                if (rso.next()) {
                    ...
                    val email = ""
                    val phone = ""
                    val contactPerson = htmlTen.selectFirst("td:contains(Ответственное лицо:) + td")?.ownText()?.trim { it <= ' ' }
                            ?: ""
            ...
            val delivPlace = htmlTen.selectFirst("td:contains(Адрес места поставки товара) + td")?.ownText()?.trim { it <= ' ' }
                    ?: ""
