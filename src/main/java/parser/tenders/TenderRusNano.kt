    ...
    override fun parsing() {
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(fun(con: Connection) {
                val stmt0 =
                    con.prepareStatement("SELECT id_tender FROM ${BuilderApp.Prefix}tender WHERE purchase_number = ? AND doc_publish_date = ? AND type_fz = ? AND end_date = ?")
                        .apply {
                ...
                if (tn.orgName != "") {
                    ...
                    if (rso.next()) {
                        ...
                        val phone = ""
                        val contactPerson =
                            htmlTen.selectFirst("td:contains(Ответственное лицо:) + td")?.ownText()?.trim { it <= ' ' }
                                ?: ""
                ...
                var idLot = 0
                val lotNumber = 1
                val currency = htmlTen.selectFirst("td:contains(Вид валюты:) + td")?.ownText()?.trim { it <= ' ' }
                    ?: ""
                val nmck = (htmlTen.selectFirst("td:contains(Общая стоимость:) + td")?.ownText()?.trim { it <= ' ' }
                    ?: "").extractPrice()
                ...
                val delivPlace =
                    htmlTen.selectFirst("td:contains(Адрес места поставки товара) + td")?.ownText()?.trim { it <= ' ' }
                        ?: ""
                val delivTerm = htmlTen.selectFirst("td:contains(Условия поставки) + td")?.ownText()?.trim { it <= ' ' }
                    ?: ""
                ...
                val okpdName = htmlTen.selectFirst("td:contains(Категория ОКПД2) + td")?.text()?.trim { it <= ' ' }
                    ?: ""
                val insertPurObj =
                    con.prepareStatement("INSERT INTO ${BuilderApp.Prefix}purchase_object SET id_lot = ?, id_customer = ?, name = ?, sum = ?, okpd_name = ?")
                        .apply {
                            setInt(1, idLot)
                            setInt(2, idCustomer)
