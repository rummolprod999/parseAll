...
class TenderEnPlusAst(val drv: ChromeDriver) : TenderAbstract(), ITender {
    ...
    override fun parsing() {
        ...
        val status = drv.findElementWithoutException(By.xpath("//td[contains(., 'Статус')]/following-sibling::td/span"))?.text?.trim { it <= ' ' }
                ?: ""
        ...
        val dateScoringTmp = drv.findElementWithoutException(By.xpath("//tbody[@content = 'other:TenderInfobody']//td[contains(., 'Дата и время рассмотрения заявок')]/following-sibling::td/span"))?.text?.trim()?.trim { it <= ' ' }
                ?: ""
        val dateScoring = dateScoringTmp.getDateFromString(formatterGpn)
        val dateBiddingTmp = drv.findElementWithoutException(By.xpath("//tbody[@content = 'other:TenderInfobody']//td[contains(., 'Дата и время начала торгов')]/following-sibling::td/span"))?.text?.trim()?.trim { it <= ' ' }
                ?: ""
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb).use(fun(con: Connection) {
            ...
            if (fullnameOrg != "") {
                ...
                if (rso.next()) {
                    ...
                    rso.close()
                    stmto.close()
                    val postalAdr = drv.findElementWithoutException(By.xpath("//td[contains(., 'Место нахождения')]/following-sibling::td/span"))?.text?.trim()?.trim { it <= ' ' }
                            ?: ""
                    val factAdr = drv.findElementWithoutException(By.xpath("//td[contains(., 'Почтовый адрес')]/following-sibling::td/span"))?.text?.trim()?.trim { it <= ' ' }
                            ?: ""
    ...
    private fun parserLot(el: WebElement, con: Connection, href: String, lotNum: Int) {
        ...
        if (!customers.isEmpty()) {
            val cusName = customers[0].findElementWithoutException(By.xpath("./td[3]/span"))?.text?.trim()?.trim { it <= ' ' }
                    ?: ""
            val cusInn = customers[0].findElementWithoutException(By.xpath("./td[4]/span"))?.text?.trim()?.trim { it <= ' ' }
                    ?: ""
            if (cusName != "") {
        ...
        val delivPlace = el.findElementWithoutException(By.xpath(".//td[contains(., 'Количество, место, сроки (период)')]/following-sibling::td/span"))?.text?.trim()?.trim { it <= ' ' }
                ?: ""
        val delivTerm = el.findElementWithoutException(By.xpath(".//td[contains(., 'Условие подачи заявки на участие')]/following-sibling::td/span"))?.text?.trim()?.trim { it <= ' ' }
                ?: ""
        ...
        if (!purObjects.isEmpty()) {
            for (po in purObjects) {
                val name = po.findElementWithoutException(By.xpath("./td[4]/span"))?.text?.trim()?.trim { it <= ' ' }
                        ?: ""
                val quantity = po.findElementWithoutException(By.xpath("./td[5]/span"))?.text?.trim()?.deleteAllWhiteSpace()?.trim { it <= ' ' }
                        ?: ""
                ...
                val okei = po.findElementWithoutException(By.xpath("./td[9]/span"))?.text?.trim()?.trim { it <= ' ' }
                        ?: ""
                val price = po.findElementWithoutException(By.xpath("./td[6]/span"))?.text?.trim()?.replace(',', '.')?.deleteAllWhiteSpace()?.trim { it <= ' ' }
                        ?: ""
                con.prepareStatement("INSERT INTO ${BuilderApp.Prefix}purchase_object SET id_lot = ?, id_customer = ?, name = ?, okei = ?, quantity_value = ?, customer_quantity_value = ?, okpd_name = ?, price = ?").apply {
                    setInt(1, idLot)
                    setInt(2, idCustomer)
                    setString(3, name)
                    setString(4, okei)
