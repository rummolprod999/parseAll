...
class TenderZmoNen(val tn: ZmoKursk) : TenderAbstract(), ITender {
    init {
        etpName = "Электронный магазин Ненецкого автономного округа"
        etpUrl = "https://naomarket-app.rts-tender.ru/"
    }
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(
                fun(con: Connection) {
                    ...
                    val idPlacingWay = 0
                    var idTender = 0
                    val idRegion = getIdRegion(con, "ненец")
                    val insertTender =
                        con.prepareStatement(
