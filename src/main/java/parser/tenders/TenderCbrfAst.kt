...
class TenderCbrfAst(val drv: ChromeDriver) : TenderAbstract(), ITender {
    ...
    override fun parsing() {
        ...
        if (datePubTmp == "") {
            datePubTmp =
                drv.findElementWithoutException(By.xpath("//td[contains(., 'Дата и время начала подачи заявок на участие')]/following-sibling::td/span"))?.text?.trim()
                    ?.trim { it <= ' ' }
                    ?: ""
        }
        var dateEndTmp =
            drv.findElementWithoutException(By.xpath("//td[contains(., 'Дата и время окончание подачи заявок на участие')]/following-sibling::td/span"))?.text?.trim()
                ?.trim { it <= ' ' }
                ?: ""
        if (dateEndTmp == "") {
            dateEndTmp =
