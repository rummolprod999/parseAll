            ...
            .use(fun(con: Connection) {
                ...
                if (fullnameOrg != "") {
                    ...
                    if (rso.next()) {
                        ...
                        stmto.close()
                        val postalAdr =
                            driver.findElementWithoutException(By.xpath("//td[contains(., 'Почтовый адрес:')]/following-sibling::td"))?.text?.trim()
                                ?.trim { it <= ' ' }
                                ?: ""
                        val factAdr =
                            driver.findElementWithoutException(By.xpath("//td[contains(., 'Юридический адрес:')]/following-sibling::td"))?.text?.trim()
                                ?.trim { it <= ' ' }
                                ?: ""
                        val inn = ""
                        val kpp = ""
                        val email =
                            driver.findElementWithoutException(By.xpath("//td[contains(., 'Адрес электронной почты:')]/following-sibling::td"))?.text?.trim()
                                ?.trim { it <= ' ' }
                                ?: ""
                        val phone =
                            driver.findElementWithoutException(By.xpath("//td[contains(., 'Контактный телефон:')]/following-sibling::td"))?.text?.trim()
                                ?.trim { it <= ' ' }
                                ?: ""
                        val contactPerson = ""
                ...
                var idPlacingWay = 0
                val placingWayName =
                    driver.findElementWithoutException(By.xpath("//td[contains(., 'Тип процедуры:')]/following-sibling::td"))?.text?.trim()
                        ?.trim { it <= ' ' }
                        ?: ""
                if (placingWayName != "") {
                ...
                val regionName =
                    driver.findElementWithoutException(By.xpath("//td[contains(., 'Место поставки:')]/following-sibling::td"))?.text?.trim()
                        ?.trim { it <= ' ' }
                        ?: ""
                ...
                val lots =
                    driver.findElements(By.xpath("//div[contains(@class, 'x-tab-panel-body')]/div[contains(@id, 'ext-comp-') and contains(@class, 'x-panel x-panel-noborder')]/div[@class = 'x-panel-bwrap']"))
                lots.forEachIndexed { index, lot ->
                    val lotNum = index + 1
                    val nmck =
                        lot.findElementWithoutException(By.xpath(".//td[contains(., 'Начальная цена без учета НДС:')]/following-sibling::td"))?.text?.deleteAllWhiteSpace()
                            ?.replace(",", ".")?.trim()?.trim { it <= ' ' }
                            ?: ""
                    val currency =
                        lot.findElementWithoutException(By.xpath(".//td[contains(., 'Валюта:')]/following-sibling::td"))?.text?.trim()
                            ?.trim { it <= ' ' }
                            ?: ""
                    var idLot = 0
                    ...
                    var idCustomer = 0
                    val cusName =
                        lot.findElementWithoutException(By.xpath(".//td[contains(., 'Наименование заказчика:')]/following-sibling::td"))?.text?.trim()
                            ?.trim { it <= ' ' }
                            ?: ""
                    if (cusName != "") {
                    ...
                    val delivPlace =
                        lot.findElementWithoutException(By.xpath(".//td[contains(., 'Место поставки:')]/following-sibling::td"))?.text?.trim()
                            ?.trim { it <= ' ' }
                            ?: ""
                    val delivTerm =
                        lot.findElementWithoutException(By.xpath(".//td[contains(., 'Условия, сроки поставки и оплаты:')]/following-sibling::td"))?.text?.trim()
                            ?.trim { it <= ' ' }
                            ?: ""
                    ...
                    val purObjects =
                        driver.findElements(By.xpath(".//div[@class = 'x-fieldset-body x-fieldset-body-noheader']"))
                    for (po in purObjects) {
                        val poName =
                            po.findElementWithoutException(By.xpath(".//td[contains(., 'Наименование:')]/following-sibling::td"))?.text?.trim()
                                ?.trim { it <= ' ' }
                                ?: ""
                        if (poName == "") continue
                        val quant =
                            po.findElementWithoutException(By.xpath(".//td[contains(., 'Количество:')]/following-sibling::td"))?.text?.trim()
                                ?.trim { it <= ' ' }
                                ?: ""
                        val okei =
                            po.findElementWithoutException(By.xpath(".//td[contains(., 'Единица измерения:')]/following-sibling::td"))?.text?.trim()
                                ?.trim { it <= ' ' }
                                ?: ""
