...
class TenderAfkAst(val drv: ChromeDriver) : TenderAbstract(), ITender {

    var idTender = 0

    init {
        etpName = "«Сбербанк - Автоматизированная система торгов» Закупки ПАО АФК «Система»"
        etpUrl = "http://utp.sberbank-ast.ru/AFK/List/PurchaseList/"
    }
    ...
    private fun parserLot(el: WebElement, con: Connection, href: String, lotNum: Int) {
        ...
        if (!customers.isEmpty()) {
            val cusName =
                customers[0].findElementWithoutException(By.xpath("./td[2]/span"))?.text?.trim()?.trim { it <= ' ' }
                    ?: ""
            val cusInn =
        ...
        if (!purObjects.isEmpty()) {
            for (po in purObjects) {
                val name = po.findElementWithoutException(By.xpath("./td[3]/span"))?.text?.trim()?.trim { it <= ' ' }
                    ?: ""
                val quantity =
                    po.findElementWithoutException(By.xpath("./td[4]/span"))?.text?.trim()?.deleteAllWhiteSpace()
                        ?.trim { it <= ' ' }
                        ?: ""
                val okpd2 = po.findElementWithoutException(By.xpath("./td[5]/span"))?.text?.trim()?.trim { it <= ' ' }
                    ?: ""
                val okei = po.findElementWithoutException(By.xpath("./td[7]/span"))?.text?.trim()?.trim { it <= ' ' }
                    ?: ""
                con.prepareStatement("INSERT INTO ${BuilderApp.Prefix}purchase_object SET id_lot = ?, id_customer = ?, name = ?, okei = ?, quantity_value = ?, customer_quantity_value = ?, okpd2_code = ?")
                    .apply {
                        setInt(1, idLot)
                        setInt(2, idCustomer)
                        setString(3, name)
