...
class TenderUmzMark(val tn: UmzMark) : TenderAbstract(), ITender {
    ...
    override fun parsing() {
        ...
        val datePubT =
            htmlTen.selectFirst("th:contains(Дата и время создания маркетингового исследования) + td > span")?.ownText()
                ?.trim { it <= ' ' }
                ?: ""
        ...
        val purNum =
            htmlTen.selectFirst("th:contains(Номер маркетингового исследования) + td > span")?.ownText()
                ?.trim { it <= ' ' }
                ?: ""
        if (purNum == "") {
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(fun(con: Connection) {
                ...
                var delivTerm1 =
                    htmlTen.selectFirst("th:contains(Дата начала срока поставки товаров) + td > span")
                        ?.ownText()
                        ?.trim { it <= ' ' }
                        ?: ""
                var delivTerm2 =
                    htmlTen.selectFirst("th:contains(Дата окончания срока поставки товаров) + td > span")
                        ?.ownText()
                        ?.trim { it <= ' ' }
                        ?: ""
                var delivTerm3 =
                    htmlTen.selectFirst("th:contains(Требования к порядку поставки товаров) + td > span")
                        ?.ownText()
                        ?.trim { it <= ' ' }
                        ?: ""
                var delivTerm4 =
                    htmlTen.selectFirst("th:contains(Порядок оплаты) + td > span")
                        ?.ownText()
                        ?.trim { it <= ' ' }
                        ?: ""
                val delivTerm =
                ...
                val contrGuarantAmount =
                    htmlTen.selectFirst("th:contains(Размер обеспечения исполнения контракта) + td > span")
                        ?.ownText()
                        ?.trim { it <= ' ' }
                        ?: ""
                if (delivTerm != "") {
                    con.prepareStatement("INSERT INTO ${BuilderApp.Prefix}customer_requirement SET id_lot = ?, id_customer = ?, delivery_place = ?, delivery_term = ?, contract_guarantee_amount = ?")
                        .apply {
                            setInt(1, idLot)
                            setInt(2, idCustomer)
                            setString(3, "")
