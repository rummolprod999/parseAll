...
class TenderRenCredit(val tn: RenCredit) : TenderAbstract(), ITender {


    init {
        etpName = "КБ «Ренессанс Кредит»"
        etpUrl = "https://rencredit.ru/"
    }
    ...
    override fun parsing() {
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(fun(con: Connection) {
                ...
                val documents: Elements = htmlTen.select("a.document__title-link")
                documents.forEach { doc ->
                    val hrefT = doc?.attr("href")?.trim { it <= ' ' } ?: ""
                    val href = "https://rencredit.ru$hrefT"
