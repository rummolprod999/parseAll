...
class TenderZmoKomi(val tn: ZmoKursk) : TenderAbstract(), ITender {
    init {
        etpName = "Закупки малого объема Республики Коми"
        etpUrl = "https://komimarket-app.rts-tender.ru/"
    }
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(
                fun(con: Connection) {
                    ...
                    val idPlacingWay = 0
                    var idTender = 0
                    val idRegion = getIdRegion(con, "коми")
                    val insertTender =
                        con.prepareStatement(
