...
class TenderVipAst(val drv: ChromeDriver) : TenderAbstract(), ITender {
    ...
    init {
        etpName = "«Сбербанк - Автоматизированная система торгов» - Закупки коммерческих Заказчиков"
        etpUrl = "http://utp.sberbank-ast.ru/VIP/List/PurchaseList"
    }
            ...
            .use(fun(con: Connection) {
                ...
                if (fullnameOrg != "") {
                    ...
                    if (rso.next()) {
                        ...
                        stmto.close()
                        val postalAdr =
                            drv.findElementWithoutException(By.xpath("//td[contains(., 'Фактический адрес (почтовый)')]/following-sibling::td/span"))?.text?.trim()
                                ?.trim { it <= ' ' }
                                ?: ""
                        val factAdr =
                            drv.findElementWithoutException(By.xpath("//td[contains(., 'Юридический адрес')]/following-sibling::td/span"))?.text?.trim()
                                ?.trim { it <= ' ' }
                                ?: ""
                        val inn =
    ...
    private fun parserLot(el: WebElement, con: Connection, href: String, lotNum: Int, drv: ChromeDriver) {
        val nmck =
        ...
        var idCustomer = 0
        val cusName =
            drv.findElementWithoutException(By.xpath("//td[contains(., 'Полное наименование заказчика')]/following-sibling::td/span"))?.text?.trim()
                ?.trim { it <= ' ' }
                ?: ""
        val cusInn =
            drv.findElementWithoutException(By.xpath("//td[contains(., 'ИНН заказчика')]/following-sibling::td/span"))?.text?.trim()
                ?.trim { it <= ' ' }
                ?: ""
        if (cusName != "") {
            val stmtoc =
        ...
        if (!purObjects.isEmpty()) {
            for (po in purObjects) {
                val name =
                    po.findElementWithoutException(By.xpath("./td[contains(@id, 'PositionName')]/span"))?.text?.trim()
                        ?.trim { it <= ' ' }
                        ?: ""
                val quantity =
                    po.findElementWithoutException(By.xpath("./td[contains(@id, 'tdQuantity')]/span"))?.text?.trim()
                        ?.deleteAllWhiteSpace()?.trim { it <= ' ' }
                        ?: ""
                val okpd2 = ""
                val okei =
                    po.findElementWithoutException(By.xpath("./td[contains(@id, 'tdPositionUnit')]/span"))?.text?.trim()
                        ?.trim { it <= ' ' }
                        ?: ""
                val price =
                    po.findElementWithoutException(By.xpath("./td[contains(@id, 'tdMaxPrice')]/span"))?.text?.trim()
