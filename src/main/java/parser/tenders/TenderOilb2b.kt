    ...
    override fun parsing() {
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(fun(con: Connection) {
                ...
                tn.products.forEach {
                    val insertPurObj =
                        con.prepareStatement("INSERT INTO ${BuilderApp.Prefix}purchase_object SET id_lot = ?, id_customer = ?, name = ?, sum = ?, okpd_name = ?, okei = ?, quantity_value = ?, customer_quantity_value = ?")
                            .apply {
                                setInt(1, idLot)
                                setInt(2, idCustomer)
