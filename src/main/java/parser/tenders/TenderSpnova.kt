...
class TenderSpnova(val tn: Spnova) : TenderAbstract(), ITender {

    init {
        etpName = "ООО «Нова»"
        etpUrl = "http://www.snpnova.com/"
    }

    val typeFz by lazy { 317 }
                ...
                fun(con: Connection) {
                    ...
                    if (fullnameOrg != "") {
                        ...
                        if (rso.next()) {
                            ...
                            stmto.close()
                            val postalAdr =
                                "446218 , Самарская область, г.Новокуйбышевск, ул. Дзержинского, 36"
                            val inn = ""
                            val kpp = ""
                            val email = ""
                            val phone = "84635) 36-215"
                            val contactPerson = ""
                            val stmtins =
                                con.prepareStatement(
    ...
    private fun updateVersion(con: Connection, dateVer: Date): TenderUzex.Result {
        var updated1 = false
        var cancelstatus1 = 0
        val stmt =
            con.prepareStatement(
