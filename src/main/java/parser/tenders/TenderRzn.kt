...
class TenderRzn(val tn: ZmoKursk) : TenderAbstract(), ITender {
    init {
        etpName = "ЭЛЕКТРОННЫЙ МАГАЗИН ГОРОДА РЯЗАНИ"
        etpUrl = "https://ryazanvitrinatorgov-app.rts-tender.ru/"
    }
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(
                fun(con: Connection) {
                    ...
                    val idPlacingWay = 0
                    var idTender = 0
                    val idRegion = getIdRegion(con, "рязан")
                    val insertTender =
                        con.prepareStatement(
