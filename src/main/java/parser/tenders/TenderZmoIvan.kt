...
class TenderZmoIvan(val tn: ZmoKursk) : TenderAbstract(), ITender {
    init {
        etpName = "ЭЛЕКТРОННЫЙ МАРКЕТ ДЕПАРТАМЕНТА КОНКУРСОВ И АУКЦИОНОВ ИВАНОВСКОЙ ОБЛАСТИ"
        etpUrl = "https://ivanovobl-zmo.rts-tender.ru/"
    }
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(
                fun(con: Connection) {
                    ...
                    val idPlacingWay = 0
                    var idTender = 0
                    val idRegion = getIdRegion(con, "иванов")
                    val insertTender =
                        con.prepareStatement(
