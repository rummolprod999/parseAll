...
class TenderBidAst(val drv: ChromeDriver) : TenderAbstract(), ITender {
    ...
    override fun parsing() {
        ...
        val pubDate = if (datePubTmp.length <11) datePubTmp.getDateFromString(formatterOnlyDate) else datePubTmp.getDateFromString(
            formatterGpn
        )
        val endDate = if (dateEndTmp.length <11) dateEndTmp.getDateFromString(formatterOnlyDate) else dateEndTmp.getDateFromString(
            formatterGpn
        )
