...
class TenderRussianPostAst(val drv: ChromeDriver) : TenderAbstract(), ITender {
    ...
    override fun parsing() {
        ...
        val status =
            drv.findElementWithoutException(By.xpath("//td[contains(., 'Статус процедуры')]/following-sibling::td/span"))?.text?.trim { it <= ' ' }
                ?: ""
        val purName =
        ...
        var datePubTmp =
            drv.findElementWithoutException(By.xpath("//td[contains(., 'Дата и время начала срока подачи предложений')]/following-sibling::td/span"))?.text?.trim()
                ?.trim { it <= ' ' }
                ?: ""
        if (datePubTmp == "") {
            datePubTmp =
        ...
        var dateEndTmp =
            drv.findElementWithoutException(By.xpath("//td[contains(., 'Дата и время окончания срока подачи предложений')]/following-sibling::td/span"))?.text?.trim()
                ?.trim { it <= ' ' }
                ?: ""
        if (dateEndTmp == "") {
            dateEndTmp =
        ...
        val dateScoringTmp =
            drv.findElementWithoutException(By.xpath("//td[contains(., 'Окончание рассмотрения заявок на участие')]/following-sibling::td/span"))?.text?.trim()
                ?.trim { it <= ' ' }
                ?: ""
            ...
            .use(fun(con: Connection) {
                ...
                if (fullnameOrg != "") {
                    ...
                    if (rso.next()) {
                        ...
                        stmto.close()
                        val postalAdr =
                            drv.findElementWithoutException(By.xpath("//td[contains(., 'Почтовый адрес (фактический адрес)')]/following-sibling::td/span"))?.text?.trim()
                                ?.trim { it <= ' ' }
                                ?: ""
                        val factAdr =
                            drv.findElementWithoutException(By.xpath("//td[contains(., 'Место нахождения (юридический адрес)')]/following-sibling::td/span"))?.text?.trim()
                                ?.trim { it <= ' ' }
                                ?: ""
                        val inn =
    ...
    private fun parserLot(el: WebElement, con: Connection, href: String, lotNum: Int, drv: ChromeDriver): Boolean {
        val nmck =
            el.findElementWithoutException(By.xpath(".//td[contains(., 'Начальная цена')]/following-sibling::td/span"))?.text?.replace(
                ',',
                '.'
        ...
        val lotName =
            el.findElementWithoutException(By.xpath(".//td[contains(., 'Наименование лота')]/following-sibling::td/span"))?.text?.trim()
                ...
                ?: ""
        val currency =
            el.findElementWithoutException(By.xpath(".//td[contains(., 'Валюта')]/following-sibling::td/span"))?.text?.trim()
                ?.trim { it <= ' ' }
                ?: ""
        var idLot = 0
        ...
        var idCustomer = 0
        val cusName =
            drv.findElementWithoutException(By.xpath("//td[contains(., 'Наименование заказчика')]/following-sibling::td/span"))?.text?.trim()
                ?.trim { it <= ' ' }
                ?: ""
        val cusInn =
            drv.findElementWithoutException(By.xpath("//td[contains(., 'ИНН')]/following-sibling::td/span"))?.text?.trim()
                ?.trim { it <= ' ' }
                ?: ""
        if (cusName != "") {
            val stmtoc =
        ...
        val delivPlace =
            el.findElementWithoutException(By.xpath(".//td[contains(., 'Место поставки товара, выполнения работ, оказания услуг')]/following-sibling::td/span"))?.text?.trim()
                ?.trim { it <= ' ' }
                ?: ""
        val delivTerm =
            el.findElementWithoutException(By.xpath(".//td[contains(., 'Сроки поставки товара, выполнения работ, оказания услуг')]/following-sibling::td/span"))?.text?.trim()
                ?.trim { it <= ' ' }
                ?: ""
        val appGuarantAmount =
            el.findElementWithoutException(By.xpath(".//td[contains(., 'Размер обеспечения заявки (задатка) на площадке, руб')]/following-sibling::td/span"))?.text?.replace(
                ',',
                '.'
            )?.deleteAllWhiteSpace()?.trim { it <= ' ' }
                ?: ""
        ...
        if (purObjects.isNotEmpty()) {
            for (po in purObjects) {
                val name =
                    po.findElementWithoutException(By.xpath(".//span[@content = 'leaf:BidPositionName']"))?.text?.trim()
                        ?.trim { it <= ' ' }
                        ?: ""
                val quantity =
                    po.findElementWithoutException(By.xpath(".//span[@content = 'leaf:BidPositionQuantity']"))?.text?.trim()
                        ?.deleteAllWhiteSpace()?.trim { it <= ' ' }
                        ?: ""
                val okpd2 =
                    po.findElementWithoutException(By.xpath(".//span[@content = 'leaf:BidPositionOKPD2Name']"))?.text?.trim()
                        ?.trim { it <= ' ' }
                        ?: ""
                val okei =
                    po.findElementWithoutException(By.xpath(".//span[@content = 'leaf:BidPositionOKEIName']"))?.text?.trim()
                        ?.trim { it <= ' ' }
                        ?: ""
                val price =
                    po.findElementWithoutException(By.xpath(".//span[@content = 'leaf:BidPositionPrice']"))?.text?.trim()
                        ?.replace(',', '.')?.deleteAllWhiteSpace()?.trim { it <= ' ' }
                        ?: ""
                con.prepareStatement("INSERT INTO ${BuilderApp.Prefix}purchase_object SET id_lot = ?, id_customer = ?, name = ?, okei = ?, quantity_value = ?, customer_quantity_value = ?, okpd2_code = ?, price = ?")
                    .apply {
                        setInt(1, idLot)
                        setInt(2, idCustomer)
                        setString(3, name)
