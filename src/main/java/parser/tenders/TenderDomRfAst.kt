...
class TenderDomRfAst(val drv: ChromeDriver) : TenderAbstract(), ITender {
    ...
    override fun parsing() {
        ...
        val status =
            drv.findElementWithoutException(
                    By.xpath("//td[contains(., 'Статус')]/following-sibling::td/span")
                )
                ?.text
                ?.trim { it <= ' ' } ?: ""
        val purName =
        ...
        val dateScoringTmp =
            drv.findElementWithoutException(
                    By.xpath(
                        "//td[contains(., 'Дата рассмотрения заявок')]/following-sibling::td/span"
                    )
                )
                ?.text
                ?.trim()
            ...
            .use(
                fun(con: Connection) {
                    ...
                    val fullnameOrg =
                        drv.findElementWithoutException(
                                By.xpath(
                                    "//tbody[@content = 'other:OrganizatorInfobody']//td[contains(., 'Наименование')]/following-sibling::td"
                                )
                            )
                            ?.text
                            ?.trim()
                    ...
                    if (fullnameOrg != "") {
                        ...
                        if (rso.next()) {
                            ...
                            val factAdr =
                                drv.findElementWithoutException(
                                        By.xpath(
                                            "//td[contains(., 'Почтовый адрес')]/following-sibling::td/span"
                                        )
                                    )
                                    ?.text
                                    ?.trim()
                            ...
                            val inn =
                                drv.findElementWithoutException(
                                        By.xpath(
                                            "//tbody[@content = 'other:OrganizatorInfobody']//td[contains(., 'ИНН')]/following-sibling::td"
                                        )
                                    )
                                    ?.text
                                    ?.trim()
                            ...
                            val kpp =
                                drv.findElementWithoutException(
                                        By.xpath(
                                            "//tbody[@content = 'other:OrganizatorInfobody']//td[contains(., 'КПП')]/following-sibling::td"
                                        )
                                    )
                                    ?.text
                                    ?.trim()
                            ...
                            val email =
                                drv.findElementWithoutException(
                                        By.xpath(
                                            "//tbody[@content = 'other:ContactInfobody']//td[contains(., 'Адрес электронной почты')]/following-sibling::td"
                                        )
                                    )
                                    ?.text
                                    ?.trim()
                            ...
                            val phone =
                                drv.findElementWithoutException(
                                        By.xpath(
                                            "//tbody[@content = 'other:ContactInfobody']//td[contains(., 'Номер контактного телефона')]/following-sibling::td"
                                        )
                                    )
                                    ?.text
                                    ?.trim()
                            ...
                            val contactPerson =
                                drv.findElementWithoutException(
                                        By.xpath(
                                            "//tbody[@content = 'other:ContactInfobody']//td[contains(., 'Контактное лицо')]/following-sibling::td"
                                        )
                                    )
                                    ?.text
                                    ?.trim()
                    ...
                    val factAdrOrg =
                        drv.findElementWithoutException(
                                By.xpath(
                                    "//tbody[@content = 'other:BidPlaceInfobody']//td[contains(., 'Место поставки (адрес)')]/following-sibling::td/span"
                                )
                            )
                            ?.text
                            ?.trim()
    ...
    private fun parserLot(el: WebElement, con: Connection, href: String, lotNum: Int) {
        ...
        val currency =
            el.findElementWithoutException(
                    By.xpath(".//td[contains(., 'Валюта')]/following-sibling::td/spann")
                )
                ?.text
                ?.trim()
                ?.trim { it <= ' ' } ?: ""
        ...
        val customers =
            el.findElements(
                By.xpath(
                    ".//tbody[@content = 'other:BidCustomerInfobody']//td[contains(., 'Наименование заказчика')]/following-sibling::td/span"
                )
            )
        if (!customers.isEmpty()) {
            ...
            val cusInn =
                el.findElementWithoutException(
                        By.xpath(
                            ".//tbody[@content = 'other:BidCustomerInfobody']//td[contains(., 'ИНН')]/following-sibling::td/span"
                        )
                    )
                    ?.text
                    ?.trim()
        ...
        val delivTerm =
            el.findElementWithoutException(
                    By.xpath(
                        ".//td[contains(., 'Условие подачи заявки на участие')]/following-sibling::td/span"
                    )
                )
                ?.text
                ?.trim()
        ...
        if (!purObjects.isEmpty()) {
            for (po in purObjects) {
                val name =
                    po.findElementWithoutException(By.xpath("./td[4]/span"))?.text?.trim()?.trim {
                        it <= ' '
                    } ?: ""
                val quantity =
                    po.findElementWithoutException(By.xpath("./td[5]/span"))
                        ?.text
                        ?.trim()
                        ?.deleteAllWhiteSpace()
                        ?.trim { it <= ' ' } ?: ""
                ...
                val okei =
                    po.findElementWithoutException(By.xpath("./td[9]/span"))?.text?.trim()?.trim {
                        it <= ' '
                    } ?: ""
                val price =
                    po.findElementWithoutException(By.xpath("./td[6]/span"))
                        ?.text
                        ?.trim()
                        ?.replace(',', '.')
                        ?.deleteAllWhiteSpace()
                        ?.trim { it <= ' ' } ?: ""
                con.prepareStatement(
                        "INSERT INTO ${BuilderApp.Prefix}purchase_object SET id_lot = ?, id_customer = ?, name = ?, okei = ?, quantity_value = ?, customer_quantity_value = ?, okpd_name = ?, price = ?"
                    )
                    .apply {
                        setInt(1, idLot)
                        setInt(2, idCustomer)
    ...
    private fun parserLots(drv: ChromeDriver, con: Connection, href: String) {
        ...
        if (lots.isEmpty()) {
            lots =
                drv.findElements(
                    By.xpath(
                        "//thead[.//th[ contains(.,'Сведения о лотах')]]/following-sibling::tbody/tr"
                    )
                )
        }
        if (lots.isEmpty()) {
