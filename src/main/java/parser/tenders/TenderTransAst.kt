...
class TenderTransAst(val drv: ChromeDriver) : TenderAbstract(), ITender {

    var idTender = 0

    init {
        etpName = "«Сбербанк - Автоматизированная система торгов» Закупки ПАО \"Транснефть\""
        etpUrl = "http://utp.sberbank-ast.ru/Transneft/List/PurchaseList"
    }
    ...
    override fun parsing() {
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(fun(con: Connection) {
                ...
                var idOrganizer = 0
                val fullnameOrg =
                    drv.findElementWithoutException(By.xpath("//td[contains(., 'Наименование Организатора')]/following-sibling::td/span"))?.text?.trim()
                        ?.trim { it <= ' ' }
                        ?: ""
                if (fullnameOrg != "") {
                    val stmto =
    ...
    private fun parserLot(el: WebElement, con: Connection, href: String, lotNum: Int) {
        val nmck =
            el.findElementWithoutException(By.xpath(".//td[contains(., 'Начальная (максимальная) цена договора (лота)')]/following-sibling::td/span"))?.text?.replace(
                ',',
                '.'
