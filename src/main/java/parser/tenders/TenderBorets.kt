...
class TenderBorets(val tn: Borets) : TenderAbstract(), ITender {
    init {
        etpName = "ООО \"ПК \"Борец\""
        etpUrl = "http://tenderborets.ru/"
    }
    ...
    private fun insertDocs(htmlTen: Document, con: Connection, idTender: Int) {
        val documents = htmlTen.select("a[href ^='http://tenderborets.ru/system/files/']")
