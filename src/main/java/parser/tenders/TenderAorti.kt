                ...
                fun(con: Connection) {
                    val stmt0 =
                        con.prepareStatement(
                                "SELECT id_tender FROM ${BuilderApp.Prefix}tender WHERE purchase_number = ? AND doc_publish_date = ? AND type_fz = ?"
                            )
                            .apply {
                    ...
                    val pubDateT =
                        htmlTen.selectFirst("div.post-meta + p")?.ownText()?.trim { it <= ' ' }
