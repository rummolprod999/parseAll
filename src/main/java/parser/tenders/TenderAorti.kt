...
class TenderAorti(val tn: Achi, val datePub: Date) : TenderAbstract(), ITender {
    init {
        etpName = "АО «Концерн «РТИ Системы»"
        etpUrl = "https://www.aorti.ru/"
    }

    companion object TypeFz {
        const val typeFz = 264
    }
    ...
    override fun parsing() {
        ...
        DriverManager.getConnection(BuilderApp.UrlConnect, BuilderApp.UserDb, BuilderApp.PassDb)
            .use(fun(con: Connection) {
                val stmt0 =
                    con.prepareStatement("SELECT id_tender FROM ${BuilderApp.Prefix}tender WHERE purchase_number = ? AND doc_publish_date = ? AND type_fz = ?")
                        .apply {
                ...
                val pubDateT = htmlTen.selectFirst("div.post-meta + p")?.ownText()?.trim { it <= ' ' }
                val pubDateR = pubDateT?.getDataFromRegexp("""(\d{2}\.\d{2}\.\d{4})""")
                dateEnd = pubDateR?.getDateFromString(formatterOnlyDate) ?: dateEnd
                var cancelstatus = 0
                var updated = false
                val stmt =
    ...
    private fun insertDocs(htmlTen: Document, con: Connection, idTender: Int) {
        ...
        for (doc in documents) {
            ...
            val url = "https://www.aorti.ru$urlT"
