...
class ParserKurganKhim : IParser, ParserAbstract() {
    ...
    private fun parsingTender(e: Element) {
        val purName =
            e.selectFirst("td:eq(1) a b")?.ownText()?.trim { it <= ' ' }
                ?: run {
        ...
        val purNum =
            e.selectFirst("td:eq(0) > div:eq(0) b")?.ownText()?.replace("№", "")?.trim { it <= ' ' }
                ?: run {
        ...
        val pubDateT =
            e.selectFirst("td:eq(2) > div:eq(0)")?.ownText()?.trim { it <= ' ' }
                ?: run {
        ...
        val endDateT =
            e.selectFirst("td:eq(2) > div:eq(0) > div > div")?.ownText()?.trim { it <= ' ' }
                ?: run {
