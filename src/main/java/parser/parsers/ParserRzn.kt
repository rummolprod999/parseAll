...
import parser.tenders.TenderRzn
...
class ParserRzn : IParser, ParserAbstract() {
    private val tendersS = mutableListOf<TenderRzn>()

    companion object WebCl {
        const val BaseUrl = "https://ryazanvitrinatorgov-app.rts-tender.ru/"
    ...
    override fun parser() = parse { parserRzn() }

    private fun parserRzn() {
        var tr = 0
        while (true) {
            try {
                parserSelen()
