...
import parser.tenders.TenderZmoIvan
...
class ParserZmoIvan : IParser, ParserAbstract() {
    private val tendersS = mutableListOf<TenderZmoIvan>()

    companion object WebCl {
        const val BaseUrl = "https://ivanovobl-zmo.rts-tender.ru/"
    ...
    override fun parser() = parse { parserIvan() }

    private fun parserIvan() {
        var tr = 0
        while (true) {
            try {
                parserSelen()
