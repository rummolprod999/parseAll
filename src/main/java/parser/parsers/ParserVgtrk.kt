...
class ParserVgtrk : IParser, ParserAbstract() {
    ...
    private fun parsingTender(e: Element) {
        ...
        val purNum = e.selectFirst("td:eq(0) a")?.ownText()?.trim { it <= ' ' }
        ...
        val pubDateT = e.selectFirst("td:eq(4)")?.ownText()?.trim { it <= ' ' }
                ?: run { logger("pubDateT not found"); return }
        val datePub = pubDateT.getDateFromString(formatter)
        val endDateT = e.selectFirst("td:eq(5)")?.ownText()?.trim { it <= ' ' }
        ...
        val tn = Vgtrk(cusName = cusName, endDate = dateEnd, href = urlTend, Nmck = nmck, pubDate = datePub, purName = purName, purNum = purNum, state = state)
