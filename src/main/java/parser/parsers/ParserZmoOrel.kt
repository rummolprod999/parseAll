...
import parser.tenders.TenderZmoOrel
...
class ParserZmoOrel : IParser, ParserAbstract() {
    private val tendersS = mutableListOf<TenderZmoOrel>()

    companion object WebCl {
        const val BaseUrl = "https://zmo-orel.rts-tender.ru/"
    ...
    override fun parser() = parse { parserOrel() }

    private fun parserOrel() {
        var tr = 0
        while (true) {
            try {
                parserSelen()
