...
import parser.tenders.TenderZmoChel
...
class ParserZmoChel : IParser, ParserAbstract() {

    private val tendersS = mutableListOf<TenderZmoChel>()
    ...
    override fun parser() = parse { parserZmoChel() }
    private fun parserZmoChel() {
        var tr = 0
        while (true) {
            try {
                parserSelen()
    ...
    private fun parserTender(el: WebElement) {
        ...
        val t = TenderZmoChel(tt)
        tendersS.add(t)
    }

    companion object WebCl {
        const val BaseUrl = "https://chelyabinskmarket-app.rts-tender.ru/"
