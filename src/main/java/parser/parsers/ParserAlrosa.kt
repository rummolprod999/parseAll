...
class ParserAlrosa : IParser, ParserAbstract() {
    ...
    companion object WebCl {
        const val BaseUrl = "https://zakupki.alrosa.ru/info_pur_proc?sap-return-url=%2fsap%2fbc%2fnwbc#"
    ...
    private fun getListTenders(driver: ChromeDriver, wait: WebDriverWait): Boolean {
        ...
        loop@ while (true) {
            driver.switchTo().defaultContent()
            val tenders = driver.findElements(By.xpath("//tbody[@id = 'WD0120-contentTBody']/tr[@rr and @sst and @rt]"))
            for (it in tenders) {
                try {
    ...
    private fun parserTender(el: WebElement, driver: ChromeDriver) {
        ...
        val pwName = el.findElementWithoutException(By.xpath("./td[7]/span/span"))?.text?.trim { it <= ' ' } ?: ""
        val nmckT = el.findElementWithoutException(By.xpath("./td[12]/span/span"))?.text?.trim { it <= ' ' } ?: ""
...
//        val button = driver.findElement(By.xpath("//a[@action = 'close']//tbody/tr[@rr and @sst]"))
//        button.click()
        try {
            val js = driver as JavascriptExecutor
