...
class ParserDmtu : IParser, ParserAbstract() {
    ...
    private fun parsingTender(e: Element) {
        ...
        val purNum =
            e.selectFirst("div.number")?.ownText()?.trim { it <= ' ' }
                ?: run {
        ...
        if (purNum == "") return
        val pwName =
            e.selectFirst("div:eq(2)")?.ownText()?.trim { it <= ' ' }
                ?: run {
                    logger("purName not found")
                    return
                }
        val urlTender =
            e.selectFirst("div:eq(6) a")?.attr("href")?.trim { it <= ' ' }
                ?: run {
                    logger("urlTender not found on $purName")
                    return
                }
        val status =
            e.selectFirst("div:eq(5)")?.ownText()?.trim { it <= ' ' }
                ?: run {
                    logger("status not found")
                    return
                }
        val pubDateT =
            e.selectFirst("div.pod_zaya")?.ownText()?.trim { it <= ' ' }
                ?: run {
        ...
        val endDateT =
            e.selectFirst("div:eq(4)")?.ownText()?.trim { it <= ' ' }
                ?: run {
