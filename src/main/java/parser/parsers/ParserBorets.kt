...
class ParserBorets : IParser, ParserAbstract() {
    ...
    private fun parsingTender(e: Element) {
        val purName = e.selectFirst("h2.title > a")?.ownText()?.trim { it <= ' ' }
            ?: run { logger("purName not found"); return }
        val urlT = e.selectFirst("h2.title > a")?.attr("href")?.trim { it <= ' ' }
        ...
        val delivTerm = e.selectFirst("div.content > p")?.ownText()?.trim { it <= ' ' }
            ?: ""
        val datePubT = e.selectFirst("span.submitted")?.ownText()?.trim { it <= ' ' }
            ?: run { logger("datePubT not found"); return }
        val datePubR = datePubT.getDataFromRegexp("""в\s+(.+)""").lowercase().replaceDateBorets().replace(",", "")
        ...
        val dateEndR =
            delivTerm.getDataFromRegexp("""Срок\s+предоставления\s+предложений\s+-\s+до\s+(.+)\s+г.""").lowercase()
                .replaceDateBoretsEnd()
                .deleteAllWhiteSpace().trim()
        ...
        val cusName = e.selectFirst("img[src ^='http://tender']")?.attr("title")?.replace("Аватар пользователя", "")
            ?.trim { it <= ' ' }
            ?: ""
