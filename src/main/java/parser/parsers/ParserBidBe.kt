...
class ParserBidBe : IParser, ParserAbstract() {
        ...
        try {
            ...
            wait.until(
                ExpectedConditions.visibilityOfElementLocated(
                    By.xpath(
                        "(//div[@class = 'v-card__text' and div[@class = 'row row--dense']])[position() > 1]"
                    )
                )
            )
            Thread.sleep(5000)
    ...
    private fun parserTender(el: WebElement) {
        ...
        val purName =
            el.findElementWithoutException(By.xpath(".//a[@class = 'с-link--line']"))?.text?.trim {
                it <= ' '
            }
                ?: run {
        ...
        val tt = BidBe(purNum, href, purName, datePub, dateEnd, payMethod, delivPlace, status)
