...
class ParserDsk1 : IParser, ParserAbstract() {
    ...
    private fun parsingTender(e: Element) {
        val purName = e.selectFirst("td:eq(1)")?.ownText()?.trim { it <= ' ' }
            ?: run { logger("purName not found"); return }
        var href = e.attr("data-href")?.replace("tendery/", "") ?: throw Exception("href was not found")
        href = "https://tender.dsk1.ru$href"
        val purNum = href.getDataFromRegexp("""dsk1.ru/(\d+)/""")
        if (purNum == "") throw Exception("purNum was not found")
        val pubDateT = e.selectFirst("td:eq(3)")?.ownText()?.trim { it <= ' ' }
