...
import parser.tenders.TenderZmoKursk
...
class ParserZmoKursk : IParser, ParserAbstract() {
    ...
    override fun parser() = parse { parserZmoKursk() }
    private fun parserZmoKursk() {
        var tr = 0
        while (true) {
            try {
                parserSelen()
    ...
    private fun parserTender(el: WebElement) {
        ...
        val t = TenderZmoKursk(tt)
        tendersS.add(t)
    }

    companion object WebCl {
        const val BaseUrl = "https://zmokursk.rts-tender.ru/"
