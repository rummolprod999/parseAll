...
import parser.tenders.TenderZmoNov
...
class ParserZmoNov : IParser, ParserAbstract() {
    private val tendersS = mutableListOf<TenderZmoNov>()

    companion object WebCl {
        const val BaseUrl = "https://market-nov.rts-tender.ru/"
    ...
    override fun parser() = parse { parserNov() }

    private fun parserNov() {
        var tr = 0
        while (true) {
            try {
                parserSelen()
